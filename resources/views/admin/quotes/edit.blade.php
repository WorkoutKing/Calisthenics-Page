...
@section('content')
    <div class="container mx-auto px-6 sm:px-8 py-12">
        ...
        <div class="bg-gray-800 p-8 rounded-lg shadow-xl">
            ...
            <h1 class="text-3xl font-semibold text-white mb-6">Edit Quote</h1>
            ...
            <form action="{{ route('admin.quotes.update', $quote->id) }}" method="POST">
                ...
                <div class="mb-6">
                    ...
                    <textarea name="quote" id="quote" class="block w-full px-4 py-2 border bg-gray-700 text-white rounded-lg shadow-sm focus:outline-none focus:ring-2 focus:ring-blue-500" rows="4" required>{{ old('quote', $quote->quote) }}</textarea>
                </div>

                <!-- Author Field -->
                <div class="mb-6">
                    <label for="author" class="block text-sm font-medium text-gray-300">Author (Optional)</label>
                    <input type="text" name="author" id="author" value="{{ old('author', $quote->author) }}" class="block w-full px-4 py-2 border bg-gray-700 text-white rounded-lg shadow-sm focus:outline-none focus:ring-2 focus:ring-blue-500">
                </div>

...
@section('content')
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 mt-8" x-data="workoutForm()">
        ...
        <h1 class="text-3xl font-semibold text-gray-100 mb-6">Edit Workout</h1>

        <form action="{{ route('workouts.update', $workout->id) }}" method="POST" class="bg-gray-800 p-6 rounded-lg shadow">
            @csrf
            @method('PUT')

            <div class="mb-4">
                <label class="block text-gray-400 mb-1">Title</label>
                <input type="text" name="title" value="{{ old('title', $workout->title) }}" class="w-full px-4 py-2 rounded-lg bg-gray-700 border-none text-gray-300" required>
            </div>

            <div class="mb-4">
                <label class="block text-gray-400 mb-1">Workout Type</label>
                <input type="text" name="workout_type" value="{{ old('workout_type', $workout->workout_type) }}" class="w-full px-4 py-2 rounded-lg bg-gray-700 border-none text-gray-300" required>
            </div>

            <div class="mb-4">
                <label class="block text-gray-400 mb-1">Description</label>
                <textarea name="description" rows="4" class="w-full px-4 py-2 rounded-lg bg-gray-700 border-none text-gray-300" required>{{ old('description', $workout->description) }}</textarea>
            </div>
            ...
            <div class="mb-4">
                ...
                <select name="focus" id="focus" class="w-full px-4 py-2 rounded-lg bg-gray-700 border-none text-gray-300">
                    ...
                    <option value="Beginner" {{ old('focus', $workout->focus) == 'Beginner' ? 'selected' : '' }}>Beginner</option>
                    <option value="Intermediate" {{ old('focus', $workout->focus) == 'Intermediate' ? 'selected' : '' }}>Intermediate</option>
                    <option value="Advanced" {{ old('focus', $workout->focus) == 'Advanced' ? 'selected' : '' }}>Advanced</option>
                </select>
            </div>
                    ...
                    <select x-model="selectedExercise" class="w-full px-4 py-2 rounded-lg bg-gray-700 border-none text-gray-300">
                        ...
                        @foreach($exercises as $exercise)
                            <option value="{{ $exercise->id }}"
                                @if(in_array($exercise->id, $workout->exercises->pluck('id')->toArray())) selected @endif>
                                {{ $exercise->title }}
                            </option>
                        @endforeach
                    </select>
            ...
            @if (auth()->user()->role_id === 2)
                <div class="mb-4">
                    ...
                    <input type="text" name="seo_title" value="{{ old('seo_title', $workout->seo_title) }}" class="w-full bg-gray-700 text-white border border-gray-600 rounded-lg p-2">
                </div>

                <div class="mb-4">
                    <label for="seo_description" class="block text-white">SEO Description:</label>
                    <textarea name="seo_description" rows="3" class="w-full bg-gray-700 text-white border border-gray-600 rounded-lg p-2">{{ old('seo_description', $workout->seo_description) }}</textarea>
                </div>

                <div class="mb-4">
                    <label for="seo_keywords" class="block text-white">SEO Keywords (comma separated):</label>
                    <input type="text" name="seo_keywords" value="{{ old('seo_keywords', $workout->seo_keywords) }}" class="w-full bg-gray-700 text-white border border-gray-600 rounded-lg p-2">
                </div>
            @endif

            <button type="submit" class="bg-green-500 text-white px-4 py-2 rounded-lg hover:bg-green-600 transition">Update Workout</button>
        </form>
    </div>
    ...
    <script>
        function workoutForm() {
            return {
                selectedExercise: '',
                exercises: @json($workoutExercises), // Now we use the prepared data from the controller
                addExercise() {

...
@section('content')
    <div class="py-12">
        <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 space-y-8">
            ...
            <div class="bg-white p-6 sm:p-8 rounded-lg shadow-lg border hover:shadow-xl transition duration-200">
                <h3 class="text-2xl font-bold text-gray-800">{{ $challenge->name }}</h3>
                <p class="text-gray-700 mt-4">{{ $challenge->description }}</p>
                <div class="mt-6 grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4 text-gray-600">
                ...
                @if ($userJoined)
                    ...
                    @else
                        <div class="bg-white p-6 rounded-lg shadow-lg border hover:shadow-xl transition duration-200">
                            <h4 class="text-lg font-semibold text-gray-800">Upload Your Results</h4>
                            <form action="{{ route('challenges.complete', $challenge->id) }}" method="POST" class="mt-4">
                                @csrf
                                <div class="mb-4">
                                    <label for="result" class="block text-sm font-medium text-gray-700">Your Result Video URL</label>
                                    <input name="result" id="result" class="mt-1 block w-full px-4 py-2 border border-gray-300 rounded-lg shadow-sm focus:outline-none focus:ring-blue-500 focus:border-blue-500 sm:text-sm" required>
                                </div>

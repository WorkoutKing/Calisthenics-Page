...
<div class="container mx-auto py-8 sm:py-12 space-y-8 sm:space-y-12 px-4 sm:px-6 lg:px-8">
    ...
    <div>
        ...
        @if ($basics->isEmpty())
            <div class="bg-gray-50 text-gray-600 p-6 rounded-lg shadow-md text-center">
                <i class="fas fa-dumbbell text-4xl sm:text-5xl mb-4 text-gray-400"></i>
                <p class="text-lg sm:text-xl font-medium">No basic exercises available. Start by adding your results!</p>
            </div>
        @else
            @foreach ($basics as $basic)
                <div class="bg-white p-6 sm:p-8 rounded-lg shadow-md mb-6 flex justify-between items-center">
                    <div class="w-full sm:w-3/4">
                        <h3 class="text-2xl sm:text-3xl font-semibold text-gray-800">{{ $basic->exercise }}</h3>
                    ...
                    <i class="fas {{ $basic->approved ? 'fa-check-circle text-green-500' : 'fa-clock text-yellow-500' }} text-3xl sm:text-4xl"></i>
                </div>
            @endforeach
        @endif
    </div>
    ...
    <div class="bg-white p-6 sm:p-8 rounded-lg shadow-md">
        <h2 class="text-2xl sm:text-3xl font-semibold text-gray-800 mb-6">Upload Exercise Results</h2>
        ...
        <form action="{{ route('basics.upload') }}" method="POST" class="space-y-6">
            ...
            <!-- Exercise Selection -->
            <div>
                <label for="exercise" class="block text-lg sm:text-xl font-medium text-gray-700">Select Exercise</label>
            ...
            <!-- Reps Input -->
            <div>
                <label for="reps" class="block text-lg sm:text-xl font-medium text-gray-700">Repetitions</label>
            ...
            <!-- Video URL Input -->
            <div>
                <label for="video_url" class="block text-lg sm:text-xl font-medium text-gray-700">Video URL</label>
